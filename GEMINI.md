...
1.  **Tree Species Database:** Contains approximately 2000 tree species names with their corresponding `leaf_type` and `leaf_cycle`. It is used to automatically determine the leaf type when `species` or `genus` tags are present. The full list is available in [tree_species.csv](src/main/resources/data/tree_species.csv), and a human-readable report can be found in [docs/tree_species.md](docs/tree_species.md).
...
*   `Tilia cordata green spire` → `Tilia cordata`
*   `Platanus x acerifolia` → `Platanus × acerifolia`
*   `Tilia × europaea 'Pallida'` → `Tilia × europaea`

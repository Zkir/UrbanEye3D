...
public class RoofGeometryGenerator {
    ...
    public static Mesh generateHippedRoof(List<Point2D> basePoints, double minHeight, double wallHeight, double height, String roofOrientation) {
        Mesh mesh = new Mesh();
        if (basePoints.size() != 4) {
            // Fallback to flat roof for non-quadrilaterals
            return null;
    ...
    public static Mesh generateFlatRoof(List<List<Point2D>> contours, double minHeight, double wallHeight, double height) {
        Mesh mesh = new Mesh();

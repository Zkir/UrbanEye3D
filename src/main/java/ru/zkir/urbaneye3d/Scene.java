                ...
                if (mainContour != null && !mainContour.outerRings.isEmpty()) {
                    if (primitive instanceof Relation && mainContour.outerRings.size() > 1 && mainContour.innerRings.isEmpty()) {
                        ...
                        for (ArrayList<Point2D> outerRing : mainContour.outerRings) {
                            ...
                            var element = new RenderableBuildingElement(primitive.getPrimitiveId(), primitiveOrigin, partContour, height, minHeight, roofHeight, wallColor, roofColor, roofShape, roofDirection, roofOrientation, new HashMap<>(primitive.getInterestingTags()), primitive.get(source_key), stepHeight);
                        ...
                        var element = new RenderableBuildingElement(primitive.getPrimitiveId(), primitiveOrigin, mainContour, height, minHeight, roofHeight, wallColor, roofColor, roofShape, roofDirection, roofOrientation, new HashMap<>(primitive.getInterestingTags()), primitive.get(source_key), stepHeight);
                        renderableElements.add(element);
                    }
                }
            }
        ...
        for (OsmPrimitive primitive : dataSet.allPrimitives()) {
            if (primitive instanceof Way && primitive.hasKey("barrier")) {
                ...
                if (contour != null && !contour.outerRings.isEmpty()) {
                    ...
                    renderableElements.add(new RenderableBuildingElement(primitive.getPrimitiveId(), origin, contour, height, minHeight, 0, color, color, "flat", "", "", new HashMap<>(primitive.getInterestingTags()), null, null ));
                }
            }
        }
    }
